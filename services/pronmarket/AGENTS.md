...
| 场景 | 命令 | 工作目录 |
|------|------|----------|
| 安装依赖 | `npm install` | `services/polymarket/` |
| 启动（生产） | `npm start` | `services/polymarket/` |
| 启动（调试） | `npm run dev` | `services/polymarket/` |
| 运行测试 | `npm test` | `services/polymarket/` |
| PM2 启动 | `pm2 start ecosystem.config.js` | `services/polymarket/` |
| PM2 日志 | `pm2 logs polymarket-bot` | 任意 |
| PM2 重启 | `pm2 restart polymarket-bot` | 任意 |
