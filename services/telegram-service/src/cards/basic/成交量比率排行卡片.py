...
class 成交量比率排行卡片(RankingCard):
    ...
    def _build_keyboard(self, h):
        ...
        if show_market_row:
            kb.append([
                b(_t("btn.spot", None, lang=lang), "vr_market_spot", active=market == "spot"),
                b(_t("btn.futures", None, lang=lang), "vr_market_futures", active=market == "futures"),
            ])
        ...
        kb.append([
            b(_t("btn.desc", None, lang=lang), "vr_sort_desc", active=sort_order == "desc"),
            b(_t("btn.asc", None, lang=lang), "vr_sort_asc", active=sort_order == "asc"),
            b(_t("btn.10", None, lang=lang), "vr_limit_10", active=current_limit == 10),
            b(_t("btn.20", None, lang=lang), "vr_limit_20", active=current_limit == 20),
            b(_t("btn.30", None, lang=lang), "vr_limit_30", active=current_limit == 30),
        ])

        kb.append([
            _btn_auto(None, _t("btn.home", None, lang=lang), "ranking_menu"),
            _btn_auto(None, _t("btn.refresh", None, lang=lang), "volume_ratio_ranking_refresh"),
        ])
